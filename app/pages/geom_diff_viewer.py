...
class GeomDiffViewerWidget(QWidget):
    """
    Top-level tab for visualising the geometry displacement ΔR = R_ES − R_GS
    as arrows on the ground-state structure using the same WebGL renderer
    as the phonon and force viewers.  Available whenever R_gs + R_es are
    both present in results (Adiabatic Approximation workflow).
    """
    ...
    def __init__(self, parent=None):
        ...
        info = QLabel("Vectors: ΔR = R<sub>ES</sub> − R<sub>GS</sub>  (Å)")
        info.setTextFormat(Qt.TextFormat.RichText)
        info.setStyleSheet("font-style: italic; color: gray;")
        bar_lay.addWidget(info)
        ...
        bar_lay.addWidget(QLabel("Threshold (Å):"))
        self._thresh_spin = QDoubleSpinBox()
        self._thresh_spin.setRange(0.0, 5.0)
        ...
        self._thresh_spin.setToolTip(
            "Minimum |ΔR| in Å to draw an arrow"
        )
        ...
        self._vesta_btn.setToolTip("Export geometry difference vectors for VESTA")
        self._vesta_btn.clicked.connect(self._save_vesta)
        bar_lay.addWidget(self._vesta_btn)

        root.addWidget(bar)

        # ── hint shown before first render ────────────────────────────────────
        self._hint = QLabel(
            "Run an Adiabatic Approximation calculation to visualise "
            "ΔR = R<sub>ES</sub> − R<sub>GS</sub>."
        )
    ...
    def set_results(self, results: dict):
        ...
        self._species = self._expand_atoms(
            results.get("atoms"), len(results.get("R_gs", []))
        )
        # Auto-scale: target ~2 Å arrow for the largest displacement
        dR = results["R_es"] - results["R_gs"]
        max_norm = float(np.max(np.linalg.norm(dR, axis=1)))
        if max_norm > 1e-6:
            suggested = round(2.0 / max_norm, 2)
            self._scale_spin.setValue(float(np.clip(suggested, 0.1, 200.0)))

        self._render_btn.setEnabled(True)
        self._vesta_btn.setEnabled(results.get("lattice") is not None)
    ...
    def _delta_r(self) -> np.ndarray:
        return self._results["R_es"] - self._results["R_gs"]

    # ── render ────────────────────────────────────────────────────────────────
    def _render(self):
        r = self._results
        if r is None or "R_gs" not in r or "R_es" not in r:
            return
        ...
        payload = {
            ...
            "mode_info": {"index": 0, "energy_meV": 0.0, "Sk": 0.0},
        }
    ...
    def _save_vesta(self):
        r = self._results
        if r is None or "lattice" not in r:
            return
        ...
        content = self._build_vesta(self._delta_r(), self._scale_spin.value(), self._thresh_spin.value())
        with open(path, "w") as f:
            f.write(content)

    def _build_vesta(self, dR: np.ndarray, scale: float, thresh: float) -> str:
        r       = self._results
        lat     = r["lattice"]
        R_cart  = r["R_gs"]
        species = self._species
        ...
        lines = [
            ...
            "CRYSTAL", "",
            "TITLE",
            " Geometry difference  dR = R_ES - R_GS  —  PLUMIPY", "",
            "GROUP",
            " 1 1 P 1", "",
            "CELLP",
        ...
        vec_entries = [(i, v * scale) for i, v in enumerate(dR) if np.linalg.norm(v) >= thresh]
